...
## 输入
阶段名: $ARGUMENTS (如 understand, design, plan, implement, deliver)
...
## 第二步：规范查询（可选，MVP阶段可跳过）
### L3 企业规范
按当前任务场景查询企业知识库中的规范条目：
...
> rulers 文件作为"设计约束"和"验收条件"，在对应阶段必须加载并执行检查

## 第三步：项目知识

### understand 阶段
5. Issue 描述文件 (docs/issues/issue-{N}-*.md)
6. docs/current/BRD.md
7. docs/current/PRD.md
8. docs/knowledge/{modules}.md（按 Issue 涉及的模块按需读取）
9. docs/current/decisions/ 目录下已有 ADR
